...
async function createOrderAndSendPayment({ fromPhone, contact, business, settings, phoneId, token, wrap, product, sourceMessage }) {
  ...
  if (payLink) {
    ...
    if (!bd.bank_name&&!bd.opay_number&&!bd.palmpay_number&&!bd.kuda_number&&!bd.moniepoint_number&&!bd.cash_on_delivery) orderMsg += `📞 Please contact us for payment details.\n`;
  ...
  await supabase.from("ai_logs").insert({ business_id:business.id,contact_phone:fromPhone,incoming_msg:sourceMessage,ai_response:orderMsg,kb_hit:false,status:"success",confidence:1.0 }).catch(()=>{});
  return order;
}
...
Please follow up quickly.`).catch(()=>{});
  ...
  if (intent === "product" || intent === "price" || /\b(do you have|available|in stock|tell me about|show me|is there|in stock)\b/i.test(trimmedMsg)) {
    ...
    if (found.product) {
      ...
      await incR();
      await supabase.from("ai_logs").insert({ business_id:business.id,contact_phone:fromPhone,incoming_msg:messageText,ai_response:productMsg,kb_hit:false,status:"success",confidence:found.score }).catch(()=>{});
      return;
    }
  }
  ...
  if (settings?.ai_enabled!==false&&planLimits.ai_enabled) {
    try {
      ...
      await incR();
      await supabase.from("ai_logs").insert({ business_id:business.id,contact_phone:fromPhone,incoming_msg:messageText,ai_response:cleanText,kb_hit:false,model_used:"qwen",latency_ms:aiResult.latency,status:"success",confidence:0.85 }).catch(()=>{});
      return;
    } catch(aiErr) {
...
async function processSuccessfulOrderPayment(eventData) {
  ...
  if (phone && order) {
    ...
    if (c) await supabase.from("contacts").update({ total_orders:(c.total_orders||0)+1,total_spent:Number(c.total_spent||0)+Number(order.total||0) }).eq("id", c.id).catch(()=>{});
  }
...
app.get("/admin/all-businesses", requireAuth, async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.from("businesses")
      .select("id,username,email,phone,contact_phone,business_name,plan,reply_count,reply_limit,created_at,is_active,is_suspended,suspension_reason,email_verified")
      .order("created_at",{ascending:false});
    if (error) throw error;
    ...
    res.json({
      success:true,
      admin,
      businesses:(data||[]).map(b=>({...b,total_orders:counts[b.id]||0})),
      total:data?.length||0
    });
  } catch(err) { res.status(500).json({ error: err.message }); }
});
...
app.patch("/admin/businesses/:id/status", requireAuth, async (req, res) => {
  try {
    ...
    if (active === false || suspended === true) {
      await supabase.from("sessions").update({ is_active:false }).eq("business_id", req.params.id).catch(()=>{});
    }
...
app.post("/admin/suspend-business", requireAuth, async (req, res) => {
  try {
    ...
    if (suspended) await supabase.from("sessions").update({ is_active:false }).eq("business_id", businessId).catch(()=>{});
...
app.delete("/admin/businesses/:id", requireAuth, async (req, res) => {
  try {
    ...
    for (const table of tables) {
      await supabase.from(table).delete().eq("business_id", businessId).catch(()=>{});
    }
    await supabase.from("referrals").delete().or(`referrer_id.eq.${businessId},referred_id.eq.${businessId}`).catch(()=>{});
