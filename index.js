...
async function callQwen(message, biz, lang, intent, settings) {
  ...
  for (let i=1; i<=3; i++) {
    try {
      const r = await axios.post(`${HF_API}${HF_MODEL}`,
      ...
      let text="";
      if (Array.isArray(r.data)&&r.data[0]?.generated_text) text=r.data[0].generated_text.trim();
      else if (r.data?.generated_text) text=r.data.generated_text.trim();
      text=text.replace(/<\|im_end\|>/g,"").replace(/<\|im_start\|>/g,"").trim();
      if (!text) throw new Error("Empty response");
      return { text, latency:Date.now()-start };
    } catch(err) {
      if (i<3) { if (err.response?.status===503){await sleep(5000);continue;} if (err.response?.status===429){await sleep(10000);continue;} }
      throw err;
    }
  }
}
