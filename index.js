...
// ============================================================
const PLAN_LIMITS = {
  free:    { reply_limit:50,     product_limit:3,    contact_limit:50,    broadcast_limit:0,    kb_limit:10,   ai_enabled:true, broadcasts_enabled:false, analytics:false, excel_import:false, remove_watermark:false },
...
app.post("/dashboard/orders/:id/confirm-payment", requireAuth, async (req, res) => {
  try {
    ...
    await ignoreDb(supabase.from("payments").insert({ business_id:req.business.id,order_id:req.params.id,type:"order",amount:amountReceived||order.total,currency:order.currency,paystack_ref:reference||`MANUAL-${Date.now()}`,status:"success",plan:paymentMethod||"manual" }), "manual payment log");
...
// POST /api/subscribe/:businessId — Public upgrade checkout from pricing page
app.post("/api/subscribe/:businessId", async (req, res) => {
  try {
    ...
    const { data: biz } = await supabase.from("businesses").select("id,email,username,business_name").eq("id", businessId).single();
    if (!biz) return res.status(404).json({ error:"Business account not found." });
    if (email && String(email).toLowerCase() !== String(biz.email).toLowerCase()) {
      return res.status(400).json({ error:"Email does not match this business account." });
    }
    if (!process.env.PAYSTACK_SECRET_KEY) return res.status(500).json({ error:"Platform Paystack is not configured." });
    const response = await axios.post("https://api.paystack.co/transaction/initialize",
      { email:biz.email, amount:PLAN_PRICES_NGN[plan]*100, currency:"NGN",
        reference:`SUB-${biz.id.substring(0,8)}-${Date.now()}`,
        metadata:{ business_id:biz.id, plan, type:"subscription", username:biz.username },
...
async function callQwen(message, biz, lang, intent, settings, conversationMemory = "") {
  ...
  const sys = `You are a ${settings?.ai_personality||"friendly"} WhatsApp sales assistant for "${biz.business_name}", an African business.\nBusiness: ${biz.business_desc||"Quality products and services."}\nLocation: ${biz.city||""} ${biz.country||"Nigeria"}\nDelivery: ${biz.delivery_days||"1-3 days"} | Fee: ${biz.currency||"NGN"} ${biz.delivery_fee||0}\n${settings?.custom_instructions?`Instructions: ${settings.custom_instructions}`:""}\n${conversationMemory?`Helpful memory from this same customer's conversation only:\n${conversationMemory}\n`:""}Rules: Answer only the customer's latest message. Do not answer questions they did not ask. Keep replies under 80 words. Be warm. Use 0-2 emojis. Do not add MENU/fallback/contact text unless the customer asks for help/contact. Never make up prices, stock, delivery promises, payment status, or order status. Never say an order has been created, placed, paid, shipped, or confirmed; the VendrAI system handles real orders and payments separately. If the message asks for product/order/payment details, give only a short helpful bridge and tell them to use CATALOG, PAYMENT, or ORDER [product name]. Intent: ${intent}. Language: ${lang==="pidgin"?"Nigerian Pidgin":lang==="yo"?"Yoruba-friendly English":lang==="ha"?"Hausa-friendly English":lang==="ig"?"Igbo-friendly English":"English"}`;
