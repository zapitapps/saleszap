...
app.post("/api/subscribe/:businessId", async (req, res) => {
  try {
    ...
    if (!/^sk_(live|test)_/i.test(platformPaystackKey.trim())) return res.status(500).json({ error:"Platform Paystack secret key is invalid. Use a Paystack SECRET key that starts with sk_live_ or sk_test_, not a public key." });
