...
const PLAN_PRICES_NGN = { starter: 15000, growth: 30000, pro: 60000 };
...
app.post("/auth/register", authLimiter, async (req, res) => {
  try {
    ...
    // === AUTO SEED SAMPLE DATA ON FIRST SIGNUP (Onboarding boost)
    try {
...
// POST /dashboard/orders/:id/confirm-payment — FULL FLOW: Payment Confirmation + Auto Customer Notification (Step 6 of expanded flow)
app.post("/dashboard/orders/:id/confirm-payment", requireAuth, async (req, res) => {
  try {
    ...
    // ALWAYS send customer notification on payment confirmation (FULL LIFECYCLE requirement)
    ...
    if (contact && tok && phoneId) {
      ...
      msg += `📍 We will update you when your order ships.\n\n`;
      msg += `Thank you for shopping with us! 🙏\n`;
      msg += `Reply *TRACK ${order.order_number}* anytime for status.`;

      // Digital special handling
      ...
      await sendWA(phoneId, tok, contact.phone, msg);
    }

    res.json({ success: true, message: "✅ Payment confirmed! Customer has been automatically notified." });
  } catch (err) {
...
function hasManualPaymentDetails(bd = {}) {
  return !!(bd.bank_name || (Array.isArray(bd.bank_accounts)&&bd.bank_accounts.length) || bd.opay_number || bd.palmpay_number || bd.kuda_number || bd.moniepoint_number || bd.cash_on_delivery || (Array.isArray(bd.other_methods)&&bd.other_methods.length));
}
...
async function handleBotMessage(fromPhone, messageText, contactName, business, settings, phoneId, token, incomingMeta = {}) {
  ...
  // Payment proof handling: receipts/images are not auto-confirmed; owner must review.
...
Please send your order number too, so our team can match the payment to the right order.`;
  ...
  // Premium conversation control: handle sensitive cases before AI.
...
Please follow up quickly.`).catch(()=>{});
    ...
    if (mood === "greeting") {
    await sendWA(phoneId, token, fromPhone, wrap(buildPremiumGreeting(business, (typeof window !== "undefined" && window.currentLang) ? window.currentLang : "en"));
    await incR(); return;
  }
  ...
  // PAYMENT INFO — if there is a recent order, give that exact payment link/details
  ...
  // ORDER — create real order + payment link before AI gets involved
  ...
  // === DIRECT PATCH: FULL LIFECYCLE TRACK / RATE / REORDER (Stages 5-8) ===
  if (/^track(\s|$)/i.test(trimmedMsg) || intent === "track") {
    await handleTrackCommand(fromPhone, trimmedMsg, business, phoneId, token, wrap, contact);
    await incR();
    return;
  }
  if (/^rate\s*[1-5]/i.test(trimmedMsg) || intent === "satisfaction") {
    await handleRatingCommand(fromPhone, trimmedMsg, business, phoneId, token, wrap, contact);
    await incR();
    return;
  }
  if (/^reorder/i.test(trimmedMsg) || /^buy again/i.test(trimmedMsg)) {
    await handleReorderCommand(fromPhone, trimmedMsg, business, phoneId, token, wrap, contact);
    await incR();
    return;
  }
  ...
  // PRODUCT INTEREST — give product facts, then wait for YES/ORDER before creating order
  ...
  if (settings?.ai_enabled!==false&&planLimits.ai_enabled) {
    try {
      ...
      const aiResult = await callQwen(messageText, business, lang, intent, settings, conversationMemory);
...
app.post("/webhook/whatsapp", async (req, res) => {
  ...
  try {
    ...
    // Meta can retry the same webhook. Without this, one customer message can get two bot replies.
    const now = Date.now();
    for (const [id, ts] of processedMessages.entries()) if (now - ts > MESSAGE_DEDUPE_MS) processedMessages.delete(id);
    if (processedMessages.has(messageId)) {
      console.log(`↩️ Duplicate WhatsApp message ignored: ${messageId}`);
      return;
    }
    processedMessages.set(messageId, now);

    // Some providers can resend the same text with a different message id.
    // This prevents double replies like: AI answer + irrelevant fallback.
    ...
    if (inboundTextDedupe.has(textKey)) {
      console.log(`↩️ Duplicate WhatsApp text ignored from ${fromPhone}: "${messageText}"`);
      return;
    }
...
// ============================================================
// SALES PIPELINE CRM — Leads + 30-day follow-up
// ============================================================
...
// ============================================================
// ADMIN PANEL — Founder/team-only routes
// Roles:
//   owner   = can do everything, including deleting subscribers and managing staff
//   manager = can view/manage subscribers and change plans, but cannot delete or manage staff
//   support = can view subscribers/stats and send test messages only
//   viewer  = can view subscribers/stats only
// ============================================================
...
// === FULL LIFECYCLE: Mark Shipped (Step 5) ===
app.post("/dashboard/orders/:id/mark-shipped", requireAuth, async (req, res) => {
  try {
    ...
    if (contact && tok && phoneId) {
      ...
      msg += `🚚 Expected delivery in ${deliveryDays} days.\nReply *TRACK ${order.order_number}* for updates.`;
      await sendWA(phoneId, tok, contact.phone, msg);
    }
    res.json({ success: true, message: "✅ Order marked shipped. Customer notified." });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

// === FULL LIFECYCLE: Mark Delivered + Start Satisfaction (Step 6-7) ===
app.post("/dashboard/orders/:id/mark-delivered", requireAuth, async (req, res) => {
  try {
    ...
    if (contact && tok && phoneId) {
      ...
      msg += `How was your experience? Please reply with a rating 1-5 and any feedback.\n`;
      msg += `We value your opinion! 🙏`;
      await sendWA(phoneId, tok, contact.phone, msg);
    }
    res.json({ success: true, message: "✅ Marked delivered. Satisfaction message sent." });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

// === DIRECT PATCH: TRACK / RATE / REORDER HANDLERS (full 8-stage lifecycle) ===
async function handleTrackCommand(fromPhone, text, business, phoneId, token, wrap, contact) {
  const match = text.match(/track\s*([A-Z0-9-]+)/i);
  let orderNumber = match ? match[1] : null;

  let q = supabase.from("orders").select("*").eq("business_id", business.id);
  if (orderNumber) q = q.eq("order_number", orderNumber);
  else q = q.eq("contact_id", contact.id).order("created_at", { ascending: false }).limit(1);

  const { data: order } = await q.single();
  if (!order) {
    await sendWA(phoneId, token, fromPhone, wrap("I couldn't find any order for you. Type MENU to start shopping."));
    return;
  }

  const days = business.delivery_days || "2-5";
  let msg = `📦 *Order Status*\n\n🆔 ${order.order_number}\nStatus: *${order.status.toUpperCase()}*\n`;
  if (order.tracking_number) msg += `📍 Tracking: *${order.tracking_number}*\n`;
  if (order.status === "shipped") msg += `🚚 Expected in ${days} days.\n`;
  if (order.status === "delivered") msg += `✅ Delivered! Please rate your experience (1-5).`;
  await sendWA(phoneId, token, fromPhone, wrap(msg));
}

async function handleRatingCommand(fromPhone, text, business, phoneId, token, wrap, contact) {
  const match = text.match(/rate\s*([1-5])/i);
  const rating = match ? parseInt(match[1]) : null;
  const feedback = text.replace(/rate\s*[1-5]/i, "").trim() || null;
  ...
  if (!order) {
    await sendWA(phoneId, token, fromPhone, wrap("No recent delivered order found to rate."));
    return;
  }
  ...
  await sendWA(phoneId, token, fromPhone, wrap(`Thank you! Your rating of ${rating}/5 has been recorded. We appreciate your feedback 🙏`));
}

async function handleReorderCommand(fromPhone, text, business, phoneId, token, wrap, contact) {
  ...
  if (!lastOrder || !lastOrder.items?.length) {
    await sendWA(phoneId, token, fromPhone, wrap("No previous order found. Type ORDER [product] to start fresh."));
    return;
  }
  ...
  const newOrder = await createOrderAndSendPayment({
    ...
    token,
    wrap,
    product: lastOrder.items[0],
    sourceMessage: "REORDER"
  });

  await supabase.from("reorders").insert({
    original_order_id: lastOrder.id,
    new_order_id: newOrder.id,
    business_id: business.id,
    contact_id: contact.id
  });

  await sendWA(phoneId, token, fromPhone, wrap(`🔄 *Reorder created!*\n\nBased on your last purchase.\nCheck the payment details above.`));
}


// === AUTO SEED HELPER (used on signup + manual)
...
// === DIRECT PATCH: Sample Onboarding Data Seed Route ===
