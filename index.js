...
async function handleBotMessage(fromPhone, messageText, contactName, business, settings, phoneId, token) {
  ...
  if (wantsOrder) {
    try {
      ...
      if (matched) {
        await createOrderAndSendPayment({ fromPhone, contact, business, settings, phoneId, token, wrap, product:matched, sourceMessage:messageText });
        await incR();
        return;
      }
